...
void hrvCalculationTask(void *param) {
  while (true) {
    ...
    if (xSemaphoreTake(rrmutex, pdMS_TO_TICKS(100)) == pdTRUE) {
      // 获取RR间期数据（这里需要在源文件CheezPPG类中添加获取RR缓冲区的方法）
      // 假设添加了getRRBuffer()方法返回RingBuffer<unsigned long, 65>&
...
void setup() {
  ...
  Serial.println("Raw,Avg,Filtered,Peak,HR,LatestHRV,HRV1,HRV2,HRV3,HRV4,HRV5,HRV6");
